...
variable "enable_nat_gateway" {
  description = <<-EOT
    ...
    is false. This is required for apt package installs/upgrades in
    cloud-init to succeed unless your VPC already provides egress some
    other way (Transit Gateway, existing NAT, etc). A NAT gateway has an
    hourly charge plus per-GB data processing charges - set this to false
    if you already have egress via another path, or if enable_public_ip is
    true (in which case this is ignored).
  EOT
  type        = bool
  default     = true
}
...
variable "web_source_cidr_blocks" {
  description = "CIDR blocks allowed to reach TCP/80 and TCP/443 (Apache). Defaults to the internet since this is a web server. Only reachable at all once enable_public_ip is true."
  type        = list(string)
  default     = ["0.0.0.0/0"]
}
...
variable "ami_name_filter" {
  description = <<-EOT
    Name filter used to look up the Ubuntu 26.04 LTS minimal AMI. The
    codename segment is wildcarded since it isn't guessed here - verify
    ...
    If 26.04 isn't published yet in your region, fall back to 24.04 by
    overriding this filter.
  EOT
  type        = string
  default     = "ubuntu-minimal/images/hvm-ssd-gp3/ubuntu-*-26.04-amd64-minimal-*"
}

variable "admin_username" {
  description = "SSH username on the instance (the default cloud-init user for Canonical's Ubuntu AMIs)."
  type        = string
  default     = "ubuntu"
}
