...
variable "data_disks" {
  description = <<-EOT
    ...
    Each is initialized, brought online, and formatted (NTFS) automatically
    by the bootstrap script - no manual disk management needed in the
...
variable "additional_mounts" {
  description = <<-EOT
    ...
    (Azure Files, a generic NAS), or an S3 bucket. This does not create
    the storage itself, only mounts it - the source must already exist.
    S3 has no native Windows mount path; entries of type "s3" are skipped
    with a warning - use the AWS CLI/SDK or rclone instead.
  EOT
  type = list(object({
